...
/// The largest UDP payload that fits in a legal UDP datagram.
pub const MAX_UDP_PAYLOAD_LEN: usize = u16::MAX as usize - UDP_HEADER_LEN;
...
pub enum PacketError {
    ...
    /// The serialized UDP datagram cannot fit in the UDP length field.
    #[error("UDP datagram is too long: {length} bytes; maximum is {max} bytes")]
    UdpDatagramTooLong {
        /// Length of the supplied UDP payload.
        length: usize,
        /// Maximum UDP payload accepted by the length field.
        max: usize,
    },
    /// The IPv6 flow label exceeds its 20-bit wire representation.
    #[error("IPv6 flow label {0} exceeds the 20-bit field")]
    FlowLabelOutOfRange(u32),
}
...
impl PacketMetadata {
    /// Creates logical IPv6/UDP metadata.
    ///
    /// The flow label must be at most 20 bits. Construction itself is
    /// infallible so callers that need validation should use
    /// [`Ipv6UdpPacket::new`].
    #[must_use]
    pub const fn new(
        source: Ipv6Addr,
...
/// A complete logical IPv6 packet containing a UDP datagram.
///
/// The packet has no extension headers and is represented entirely in memory.
/// Its UDP payload is opaque to this type, allowing SCHC experiments to use
/// application data that is not CoAP.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Ipv6UdpPacket {
    bytes: Vec<u8>,
...
impl Ipv6UdpPacket {
    /// Builds a complete IPv6/UDP packet from logical header metadata.
    ///
    /// # Errors
    ///
    /// Returns an error when the flow label is not 20-bit or the UDP payload
    /// cannot fit in the UDP length field.
    pub fn new(metadata: PacketMetadata, udp_payload: &[u8]) -> PacketResult<Self> {
        if metadata.flow_label > 0x000f_ffff {
            return Err(PacketError::FlowLabelOutOfRange(metadata.flow_label));
        }
        let udp_length = udp_payload.len().checked_add(UDP_HEADER_LEN).ok_or(
            PacketError::UdpDatagramTooLong {
                length: udp_payload.len(),
                max: MAX_UDP_PAYLOAD_LEN,
            },
        )?;
        let udp_length_u16 =
            u16::try_from(udp_length).map_err(|_| PacketError::UdpDatagramTooLong {
                length: udp_payload.len(),
                max: MAX_UDP_PAYLOAD_LEN,
            })?;

        let mut udp = Vec::with_capacity(udp_length);
        udp.extend_from_slice(&metadata.source_port.to_be_bytes());
        udp.extend_from_slice(&metadata.destination_port.to_be_bytes());
        udp.extend_from_slice(&udp_length_u16.to_be_bytes());
        udp.extend_from_slice(&[0, 0]);
        udp.extend_from_slice(udp_payload);

        let checksum = compute_udp_checksum(metadata.source, metadata.destination, &udp);
        udp[6..8].copy_from_slice(&checksum.to_be_bytes());

        let mut bytes = Vec::with_capacity(IPV6_HEADER_LEN + udp_length);
        let flow_label_bytes = metadata.flow_label.to_be_bytes();
        bytes.push((IPV6_VERSION << 4) | (metadata.traffic_class >> 4));
        bytes.push((metadata.traffic_class << 4) | (flow_label_bytes[1] & 0x0f));
        ...
        bytes.push(metadata.hop_limit);
        bytes.extend_from_slice(&metadata.source.octets());
        bytes.extend_from_slice(&metadata.destination.octets());
        bytes.extend_from_slice(&udp);

        Ok(Self {
            bytes,
            metadata,
            udp_payload: udp_payload.to_vec(),
        })
    }

    /// Parses and validates a complete IPv6/UDP packet.
    ///
    /// IPv6 and UDP declared lengths must consume the complete input, the
    /// next header must be UDP, and the IPv6 UDP checksum must be valid.
    ///
    /// # Errors
    ///
    /// Returns an error for malformed IPv6/UDP input or an invalid checksum.
    pub fn parse(input: &[u8]) -> PacketResult<Self> {
        ...
        Ok(Self {
            ...
            udp_payload: udp.payload().to_vec(),
        })
    }
    ...
    /// Returns the opaque UDP payload.
    #[must_use]
    pub fn udp_payload(&self) -> &[u8] {
        &self.udp_payload
    }

    /// Returns the exact complete IPv6/UDP packet bytes.
    #[must_use]
    pub fn as_bytes(&self) -> &[u8] {
        &self.bytes
    }

    /// Copies the exact complete IPv6/UDP packet bytes.
    #[must_use]
...
impl Ipv6UdpCoapPacket {
    ...
    pub fn from_coap_datagram(
        ...
        .map_err(|error| match error {
            PacketError::UdpDatagramTooLong { length, .. } => {
                PacketError::CoapDatagramTooLong { length }
            }
            other => other,
        })?;
        Ok(Self {
    ...
    pub fn parse(input: &[u8]) -> PacketResult<Self> {
        let generic = Ipv6UdpPacket::parse(input)?;
        let coap_datagram = generic.udp_payload().to_vec();
        let coap_message = parse_coap(&coap_datagram)?;
        Ok(Self {
            bytes: generic.bytes,
            metadata: generic.metadata,
...
mod tests {
    ...
    #[test]
    fn generic_packet_preserves_non_coap_udp_payload_and_metadata() {
        let packet = Ipv6UdpPacket::new(
            ...
            b"binary UDP payload",
        )
        .expect("packet");
        let parsed = Ipv6UdpPacket::parse(packet.as_bytes()).expect("parse");
        assert_eq!(parsed.metadata(), packet.metadata());
        assert_eq!(parsed.udp_payload(), b"binary UDP payload");
    }
}
